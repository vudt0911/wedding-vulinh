<template>
	<div class="fixed bottom-[6.5rem] left-5 z-40 flex flex-col gap-y-4 md:bottom-5">
		<button class="floating-btn" type="button" title="Chia sẻ" @click="isOpen = !isOpen">
		...
		<div :class="`absolute -z-10 flex h-full w-full flex-col items-center justify-center gap-y-4 transition-transform duration-200 ${isOpen ? 'bottom-[6.5rem] -translate-y-1/2' : 'bottom-0'}`">
			...
			>
				<span class="sr-only">Chia sẻ WhatsApp</span>
			...
			>
				<span class="sr-only">Chia sẻ Twitter</span>
			...
			>
				<span class="sr-only">Chia sẻ Facebook</span>
		...
		<button class="floating-btn" type="button" :title="!isAudioPlay ? 'Mở nhạc' : 'Tắt nhạc'" @click="handleBGM">
